...
#print('C:'+os.path.sep+'Program Files (x86)'+os.path.sep+'Java'+os.path.sep+'jre1.8.0_60'+os.path.sep+'bin')
#nltk.internals.config_java('C:'+os.path.sep+'Program Files (x86)'+os.path.sep+'Java'+os.path.sep+'jre1.8.0_60'+os.path.sep+'bin')
...
def fix_tweet(tweet):
    '''Post algorithm string trimmer to fix some common issues'''
...
def get_tweet(hashtag):
    '''Returns a tweet of the hastag that is not a replica '''
...
def generate_database(hashtags,filename,n):
    '''Writes n tweets of each hashtag to the test database'''
...
if __name__ == '__main__':
    # This code is for generating to the database
    ...
    tweets = []
    num_tweets = 20
    print("Generating",num_tweets,"for #%s"%hashtag)
    while len(tweets) < 20:
        tweet = get_tweet(hashtag)
        tweets.append(tweet)
        print(tweet)
